...
const Lobby = () => {
  ...
  useEffect(() => {
    ...
    const handleRoomCreated = ({ roomId }) => {
      setRoomId(roomId); setIsWaiting(true); setIsRequestingRoom(false); setLobbyError('');
    };

    const handleRoomJoined = ({ roomId, creatorName, difficulty, company, matchType }) => {
      setRoomId(roomId); setOpponentName(creatorName); setDifficulty(difficulty);
      setCompany(company); setMatchType(matchType); setIsWaiting(true);
      setIsRequestingRoom(false); setLobbyError('');
    };

    const handlePlayerJoined = ({ joinerName }) => {
      setOpponentName(joinerName); setIsOpponentReady(false); setLobbyError('');
    };
    ...
    const handleOpponentLeft = () => {
      setOpponentName(null); setAmIReady(false); setIsOpponentReady(false);
  ...
  const resetLobby = () => {
    setIsWaiting(false); setRoomId(null); setOpponentName(null);
    setAmIReady(false); setIsOpponentReady(false); setIsRequestingRoom(false); setLobbyError('');
  };
  ...
  const handleCopy = async () => {
    ...
    try { await navigator.clipboard.writeText(roomId); } 
      ...
      const textArea = document.createElement('textarea'); textArea.value = roomId;
  ...
  const requireName = () => {
    ...
    setLobbyError('Please enter your name first.'); return false;
  };
  ...
  const handleCreateRoom = () => {
    if (!socket) { setLobbyError('Still connecting to server...'); return; }
    if (!requireName()) return;
    setLobbyError(''); setIsRequestingRoom(true);
    socket.emit('create_room', { difficulty, company, matchType: getFinalMatchType(), playerName: playerName.trim() });
  };

  const handleJoinRoom = () => {
    if (!socket) { setLobbyError('Still connecting to server...'); return; }
    if (!requireName()) return;
    if (!roomCodeInput.trim()) { setLobbyError('Please enter a room code.'); return; }
    setLobbyError(''); setIsRequestingRoom(true);
    socket.emit('join_room', { roomId: roomCodeInput.trim().toUpperCase(), playerName: playerName.trim() });
  };
  ...
  const handleCancel = () => {
    socket?.emit('leave_room', { roomId }); resetLobby(); navigate('/');
  };

  const primaryText = () => {
    if (isRequestingRoom) return 'Connecting...';
    if (isPracticeMode) return 'Start practice session ->';
    return multiplayerMode === 'create' ? 'Create race room ->' : 'Join race room ->';
  };
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%', minHeight: 'calc(100vh - 70px)', padding: '2rem' }}>
      <div style={{ width: '400px' }}>
        {!isWaiting ? (
          <LobbyForm
            isPracticeMode={isPracticeMode} multiplayerMode={multiplayerMode} playerName={playerName}
            difficulty={difficulty} company={company} matchType={matchType} customTime={customTime}
            roomCodeInput={roomCodeInput} focusedInput={focusedInput} lobbyError={lobbyError}
            isRequestingRoom={isRequestingRoom} primaryText={primaryText()} onModeChange={(mode) => { setMultiplayerMode(mode); setLobbyError(''); }}
            onPlayerNameChange={setPlayerName} onDifficultyChange={setDifficulty} onCompanyChange={setCompany}
            onMatchTypeChange={setMatchType} onCustomTimeChange={setCustomTime} onRoomCodeChange={setRoomCodeInput}
            onFocusChange={setFocusedInput} onSubmit={handleSubmit}
          />
        ) : (
          <WaitingRoom
            roomId={roomId} playerName={playerName} opponentName={opponentName}
            amIReady={amIReady} isOpponentReady={isOpponentReady} matchLabel={getFinalMatchType()}
            difficulty={difficulty} copied={copied} lobbyError={lobbyError}
            onCopy={handleCopy} onToggleReady={toggleReady} onCancel={handleCancel}
          />
        )}
      </div>
    </div>
